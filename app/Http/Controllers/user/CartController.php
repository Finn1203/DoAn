...
class CartController extends Controller
{
    public function addcart(Request $request)
    {
        ...
        $check = Cart::where([['Id_Sach', '=', $sach], ['Id_TK', '=', $IdTK]])
            ->where('TrangThai', 1)
            ->first();
    ...
    public function CheckDiscount(Request $request)
    {
        if ($request->code) {
            if (
                MaGiamGia::where('Code', '=', $request->code)
                    ->where('TrangThai', 1)
                    ->exists()
            ) {
                ...
                return [$id, $ketqua]; //ket qua , id ma giam gia
            } else {
                return false;
            }
        }
    ...
    public function checkout(Request $request)
    {
        ...
        $sach = Cart::where('Id_TK', $request->session()->get('infoUser')['id'])
            ->where('TrangThai', 1)
            ->get();
        $mytime = Carbon::now();
