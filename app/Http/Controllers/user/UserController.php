...
class UserController extends Controller
{
    ...
    public function Index()
    {
        ...
        foreach ($listcha as $cha) {
            $cha->listcon = TheLoai::where('Xoa', 0)
                ->where('TenTLCha', $cha->id)
                ->get();
        }
        ...
        $listIDs = ChiTietHoaDonBan::select('IdSach')->groupBy('IdSach')->orderByRaw('COUNT(SoLuong) DESC')->whereIn('IdHoaDB', $listIDHoadon)->take(4)->get();
    ...
    public function getNumCart()
    {
        $num_cart = 0;
        if (Session::has('infoUser')) {
            $carts = Cart::where('Id_TK', Session::get('infoUser')['id'])
                ->where('TrangThai', 1)
                ->get();
            foreach ($carts as $cart) {
                $num_cart += $cart->So_Luong;
    ...
    public function Shop(Request $request)
    {
        ...
        foreach ($tacgia as $cha) {
            $tacgias = Sach::where('Xoa', 0)
                ->where('TrangThai', 1)
                ->where('DichGia', $cha->tentacgia)
                ->get();
        }
        ...
        if (isset($_GET['start_price']) && isset($_GET['end_price']) && (isset($_GET['tacgia']) && $_GET['tacgia'] != 'null')) {
            ...
            $sach = Sach::whereBetween('GiaTien', [$min_price, $max_price])
                ->where('TrangThai', 2)
                ->where('DichGia', $request->tacgia)
                ->orderBy('GiaTien', 'ASC')
                ->paginate(12);
    ...
    public function Payment(Request $request)
    {
        ...
        if ($request->session()->has('infoUser')) {
            $cart = Cart::where('Id_TK', $request->session()->get('infoUser')['id'])
                ->where('TrangThai', 1)
                ->get();
    ...
    public function Search(Request $request)
    {
        ...
        $key = $request->keyword;
        $kq = Sach::where('TenSach', 'like', '%' . $key . '%')
            ->where('TrangThai', 2)
            ->where('Xoa', 0)
            ->get();
    ...
    public function autocomplete_ajax(Request $request)
    {
        ...
        if ($data['query']) {
            $product = Sach::where('TrangThai', 2)
                ->where('TenSach', 'LIKE', '%' . $data['query'] . '%')
                ->get();
            $output = '
            ...
            foreach ($product as $key => $val) {
                $output .=
                    '
               <li class="li_search_ajax"><a href="#">' .
                    $val->TenSach .
                    '</a></li>
               ';
            }
